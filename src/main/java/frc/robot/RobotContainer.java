...
public class RobotContainer {
  ...
  private void configureDriverBinds() {
    new Trigger(() -> Timer.getMatchTime() > 15)
    ...
    new Trigger(
            () ->
                Math.abs(xDrive.getLeftTriggerAxis())
                    > Constants.OperatorConstants.Driver.kDeadzone)
        .whileTrue(
            new ClimberAdjust(
                    () -> -xDrive.getLeftTriggerAxis(), () -> -xDrive.getLeftTriggerAxis())
    ...
    xDrive
        .a()
        .toggleOnTrue(
            new AlignToSpeaker(xDrive::getLeftY, xDrive::getLeftX, xDrive::getRightTriggerAxis));

    xDrive
        .b()
        .toggleOnTrue(
            new AlignToAmp(xDrive::getLeftY, xDrive::getLeftX, xDrive::getRightTriggerAxis));
    ...
    xDrive
        .y()
        .toggleOnTrue(
            new PathToPoint(Constants.AutoConstants.WayPoints.Blue.kAmp)
    ...
    xDrive
        .povDown()
        .onTrue(drivetrain.runOnce(drivetrain::seedFieldRelative).withName("Seed Field Relative"));
    xDrive
        .povUp()
        .toggleOnTrue(
            new MultiIntake()
                .alongWith(new Feed(), Lighting.getStrobeCommand(() -> LEDState.kPurple))
                .finallyDo(Lighting.startTimer));
  ...
  private void configureManipBinds() {
    ...
    xManip
        .a()
        .toggleOnTrue(ShootCommandFactory.getAimAndShootCommandWithWaitUntil(xManip.leftBumper()));
    ...
    xManip
        ...
        .toggleOnTrue(
            new Retract()
                .alongWith(
                    Lighting.getStrobeCommand(() -> Robot.isRed() ? LEDState.kRed : LEDState.kBlue))
                .finallyDo(Lighting.startTimer));

    xManip.povUp().toggleOnTrue(ShootCommandFactory.getPrepareAndShootCommand());
    xManip
        .povRight()
        .toggleOnTrue(ShootCommandFactory.getRapidFireCommandWithWaitUntil(xManip.leftBumper()));
    ...
    xManip
        .rightBumper()
        .whileTrue(ShootCommandFactory.getPrepareAndShootCommandWithWaitUntil(xManip.leftBumper()));

    // absolute worst case scenario
    xManip.start().and(xManip.back()).onTrue(arm.runOnce(arm::toggleArmMotorLimits));
  ...
  public void configurePitBinds() {
    ...
    // works perfectly
    xPit.y()
        .toggleOnTrue(ShootCommandFactory.getAimAndShootCommandWithWaitUntil(xPit.leftBumper()));
    ...
    // works perfectly
    xPit.povRight()
        .toggleOnTrue(ShootCommandFactory.getRapidFireCommandWithWaitUntil(xPit.leftBumper()));

    // works perfectly
    xPit.rightBumper()
        .whileTrue(ShootCommandFactory.getPrepareAndShootCommandWithWaitUntil(xPit.leftBumper()));
  ...
  private void configureSimBinds() {
    new Trigger(() -> simController.getRawButtonPressed(1))
        .toggleOnTrue(
            new AlignToSpeaker(
                () -> -simController.getRawAxis(0), () -> simController.getRawAxis(1), null));
    ...
    new Trigger(() -> simController.getRawButtonPressed(3))
        .toggleOnTrue(
            new PathToPoint(Constants.AutoConstants.WayPoints.Blue.kAmp)
                ...
                .until(
                    () ->
                        Math.abs(xDrive.getLeftX())
                                > Constants.OperatorConstants.Driver.kCommandCancelThreshold
                            || Math.abs(xDrive.getLeftY())
                                > Constants.OperatorConstants.Driver.kCommandCancelThreshold
                            || Math.abs(xDrive.getRightX())
                                > Constants.OperatorConstants.Driver.kCommandCancelThreshold)
