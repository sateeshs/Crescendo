...
public class PathPlannerCommand extends SequentialCommandGroup {
  ...
  public static void publishTrajectory(String autoName) {
    ...
    if (autoName == null) {
      Robot.teleopField
          .getObject(Constants.AutoConstants.kFieldObjectName)
    ...
    try {
      ...
      if (Robot.isRed()) {
        Robot.teleopField
            .getObject("Starting Pose")
            .setPose(
                GeometryUtil.flipFieldPose(PathPlannerAuto.getStaringPoseFromAutoFile(autoName)));
      } else {
        Robot.teleopField
            .getObject("Starting Pose")
   ...
   *
   * <p>if we want to unpublish trajectory we set auto name to null and we publish a trajectory to a
