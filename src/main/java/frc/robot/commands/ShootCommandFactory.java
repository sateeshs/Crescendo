...
public class ShootCommandFactory {
  ...
  public static Command getAimAndShootCommand() {
    return new PassToShooter()
        ...
        .andThen(
            new SetPoint()
                .raceWith(
                    new WaitUntilCommand(() -> Arm.getInstance().atSetpoint())
            ...
            new SetPoint()
                .raceWith(
                    new WaitUntilCommand(() -> Arm.getInstance().atSetpoint())
                        ...
                        .deadlineWith(new Prepare())
                        .andThen(
                            new Shoot(false).withTimeout(Constants.OperatorConstants.shootTimeout),
                            new Shoot(true)
                                .onlyWhile(() -> Intake.getInstance().getShooterSensor())))
  ...
  public static Command getAimAndShootCommandWithWaitUntil(BooleanSupplier waitUntil) {
    return new PassToShooter()
        .unless(() -> Intake.getInstance().getShooterSensor())
        .andThen(
            new SetPoint().raceWith(new Prepare().until(waitUntil).andThen(new Shoot(false))),
  ...
  public static Command getPrepareAndShootCommandWithTimeouts() {
    return new PassToShooter()
        ...
        .andThen(
            new Prepare()
                .until(() -> Shooter.getInstance().readyToShoot())
  ...
  public static Command getRapidFireCommand() {
    return new Shoot(false)
        .andThen(
            new BeltDrive(() -> Constants.IntakeConstants.beltIntakeSpeed)
  ...
  public static Command getRapidFireCommandWithWaitUntil(BooleanSupplier waitUntil) {
    return new WaitUntilCommand(waitUntil)
        .deadlineWith(new Prepare())
