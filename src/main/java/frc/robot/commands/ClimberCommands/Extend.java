...
public class Extend extends Command {
  ...
  @Override
  public void execute() {
    if (climber.getLeftPosition()
        < ClimberConstants.LeftMotor.kMaxExtensionMeters
            - ClimberConstants.LeftMotor.kExtensionThreshold) {
      climber.setLeftClimbPower(ClimberConstants.LeftMotor.kExtensionPower);
    }

    if (climber.getRightPosition()
        < ClimberConstants.RightMotor.kMaxExtensionMeters
  ...
  public boolean isFinished() {
    return (climber.getLeftPosition()
            > ClimberConstants.LeftMotor.kMaxExtensionMeters
                - ClimberConstants.LeftMotor.kExtensionThreshold)
        && (climber.getRightPosition()
            > ClimberConstants.RightMotor.kMaxExtensionMeters
                - ClimberConstants.RightMotor.kExtensionThreshold);
  }
}
