...
public class DriveToNote extends Command {
  ...
  public static BooleanSupplier thereIsANote =
      () -> LimelightHelpers.getTX(Constants.Vision.LimelightPython.llPython) != 0.00;
  ...
  @Override
  public void execute() {
    driveRequest.RotationalRate =
        pidRadianController.calculate(
            Units.degreesToRadians(
                LimelightHelpers.getTX(Constants.Vision.LimelightPython.llPython)));

    driveRequest.VelocityX = Constants.SwerveConstants.SwerveSpeeds.kMaxSpeedMetersPerSecond * 1;

    drivetrain.setControl(
        driveRequest.withRotationalRate(
            Units.degreesToRadians(
                pidRadianController.calculate(
                    LimelightHelpers.getTX(Constants.Vision.LimelightPython.llPython)))));
  }
