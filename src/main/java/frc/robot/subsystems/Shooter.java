...
public class Shooter extends SubsystemBase {
  ...
  public void periodic() {
    SmartDashboard.putNumber(
        "Shooter/Actual Top Motor RPM", topShooter.getVelocity().getValueAsDouble() * 60);
    SmartDashboard.putNumber(
        "Shooter/Actual Bottom Motor RPM", bottomShooter.getVelocity().getValueAsDouble() * 60);
  }
  ...
  private Shooter() {
    ...
    SmartDashboard.putNumber(
        "Shooter/Ready To Shoot threshold", Constants.MotorConstants.falconShooterThresholdRPM);
  ...
  public void setShoot(
      double feederDutyCycle, double topShooterDutyCycle, double bottomShooterDutyCycle) {
   ...
   *
   * <p>feedMotor does not use Velocity PID
   */
  ...
  public void shoot(double feederDutyCycle, double topShooterSetVal, double bottomShooterSetVal) {
    ...
    if (bottomShooterSetVal == 0d) {
      ...
      bottomShooter.setControl(
          shootPid.withVelocity(
              (Constants.MotorConstants.falconShooterLoadRPM * bottomShooterSetVal) / 60));
    }
    ...
    if (topShooterSetVal == 0d) {
      ...
      topShooter.setControl(
          shootPid.withVelocity(
              (Constants.MotorConstants.falconShooterLoadRPM * topShooterSetVal) / 60));
    }
  ...
  /**
   * Returns true if the shooter motors are fast enough to shoot, this function checks the left
   * motor
   */
  ...
  /**
   * Returns true if the shooter motors are fast enough to shoot, this function checks the left
   * motor and is used when the shooter isnt set to 100%
   *
  ...
  public boolean readyToShoot(double setVal) {
    ...
    return ((topShooter.getVelocity().getValueAsDouble() * 60) > threshold
        && (bottomShooter.getVelocity().getValueAsDouble() * 60) > threshold);
  }
  ...
  public void setShooterMotors(double topShooterPercentOutput, double bottomShooterPercentOutput) {
    ...
    if (topShooterPercentOutput == 0d) {
      ...
      topShooter.setControl(
          shootPid.withVelocity(
              (Constants.MotorConstants.falconShooterLoadRPM * topShooterPercentOutput) / 60));
    }
  ...
  /**
   * If the shooter is at the right angle and the shooter wheels are spinning fast enough
   *
