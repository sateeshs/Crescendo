...
public class Robot extends TimedRobot {
  ...
  public void robotInit() {
    ...
    pathToPointCommandChooser.addOption(
        "Far Amp Note", Constants.AutoConstants.WayPoints.CenterNotes.farLeft);
    pathToPointCommandChooser.addOption(
        "Far Mid Amp Note", Constants.AutoConstants.WayPoints.CenterNotes.farMidLeft);
    pathToPointCommandChooser.addOption(
        "Far Center Note", Constants.AutoConstants.WayPoints.CenterNotes.farCenter);
    pathToPointCommandChooser.addOption(
        "Far Mid Source Note", Constants.AutoConstants.WayPoints.CenterNotes.farMidRight);
    pathToPointCommandChooser.addOption(
        "Far Source Note", Constants.AutoConstants.WayPoints.CenterNotes.farRight);

    compressChooser.setDefaultOption("Compresor Enabled", true);
    compressChooser.addOption("Compressor Disabled", false);

    autoTab
        .add("Starting Pose Selector", startingPose)
        .withSize(3, 1)
        .withPosition(0, 3)
        .withWidget(BuiltInWidgets.kSplitButtonChooser);

    autoTab
        .add("Path Selector", autoChooser)
        .withSize(3, 1)
        .withPosition(0, 2)
        .withWidget(BuiltInWidgets.kComboBoxChooser);

    autoTab
        .add("Shoot Selector", autoShoot)
        .withSize(3, 1)
        .withPosition(3, 3)
        .withWidget(BuiltInWidgets.kSplitButtonChooser);

    autoTab
        .add("Center-line Sprint Path to Point Selector", pathToPointCommandChooser)
        .withSize(6, 1)
        .withPosition(0, 4)
        .withWidget(BuiltInWidgets.kSplitButtonChooser);
    ...
    autoTab
        ...
        .withProperties(
            Map.of("min_value", 0, "max_value", 15, "block increment", 1, "Divisions", 6));
    // .withProperties(Map.of("min_value", 0, "min", 0, "max_value", 15, "max", 15,
    // "block increment", 1, "divisions", 6));

    autoTab
        .addNumber("Auto Countdown", () -> Timer.getMatchTime())
        .withSize(3, 2)
        .withPosition(3, 0)
    ...
    autoTab
        .addBoolean("Ready To Shoot", () -> Shooter.getInstance().readyToShootAdvanced())
        .withSize(3, 2)
        .withPosition(0, 0)
    ...
    teleopTab
        .addBoolean(
            "Ready To Shoot",
            () ->
                Shooter.getInstance().readyToShootAdvanced()
                    && Drivetrain.getInstance().readyToShoot())
        .withSize(3, 2)
        .withPosition(0, 0);

    teleopTab
        .addNumber("Match Time", () -> Timer.getMatchTime())
        .withSize(3, 4)
        .withPosition(3, 0)
        .withWidget("Match Time")
    ...
    teleopTab
        .addBoolean("Holding", () -> Intake.getInstance().getShooterSensor())
        .withSize(3, 2)
        .withPosition(0, 2)
        .withWidget(BuiltInWidgets.kBooleanBox);
    ...
    disabledTab
        .add("Toggle Compression", compressChooser)
        .withSize(3, 1)
        .withPosition(0, 4)
        .withWidget(BuiltInWidgets.kSplitButtonChooser);

    Alert.groups.forEach(
        (group, alert) -> {
          disabledTab.add(group, alert).withSize(3, 3).withPosition(0, 0).withWidget("Alerts");
        });
    ...
    if (Robot.isReal() && Constants.Vision.UseLimelight) {
      ...
      autoTab
          .add(
              new HttpCamera(
                  Constants.Vision.LimelightPython.llPython,
    ...
    NamedCommands.registerCommand(
        "Shoot on the fly", ShootCommandFactory.getAimAndShootCommandWithTimeouts());
    NamedCommands.registerCommand(
        "Lower Arm",
        ...
        // follow path
        new SetPoint(Constants.ArmConstants.kArmMinAngle)
            .raceWith(
                new WaitUntilCommand(
                    () ->
                        Arm.getInstance()
                            .isInRangeOfTarget(Constants.ArmConstants.kArmMinAngle, 4)))
            .withName("Lowering arm to hard stop"));

    NamedCommands.registerCommand(
        "Auto Intake",
        new DeployAndIntake(true)
            .raceWith(
                new DriveToNote()
                    .onlyWhile(
                        // dont go over the halfway line
                        () ->
                            Drivetrain.getInstance().getPose().getX() + (0.92 / 2)
                                < Constants.fieldLength / 2 - 0.5))
    ...
    NamedCommands.registerCommand(
        ...
        new InfiniteIntake(0.2)
            .alongWith(
                new FullSend(0.2), Lighting.getStrobeCommand(() -> LEDState.kRobostangsOrange))
  ...
  public void driverStationConnected() {
    ...
    CommandScheduler.getInstance()
        .onCommandInitialize(
            (action) -> DataLogManager.log(action.getName() + " Command Initialized"));
    CommandScheduler.getInstance()
        .onCommandInterrupt(
            (action) -> DataLogManager.log(action.getName() + " Command Interrupted"));
    CommandScheduler.getInstance()
  ...
  public void disabledPeriodic() {
    ...
    try {
      if (!lastAuto.equals(startingPose.getSelected() + autoChooser.getSelected())) {
        pathPlannerCommand =
            AutoBuilder.buildAuto(startingPose.getSelected() + autoChooser.getSelected());
      }
  ...
  public void autonomousInit() {
    ...
    autonCommand =
        new SequentialCommandGroup(
            ...
            ShootCommandFactory.getPrepareAndShootCommandWithTimeouts()
                .onlyIf(autoShoot::getSelected),
    ...
    if (pathToPointCommandChooser.getSelected() != null) {
      autonCommand.addCommands(
          new PathToPoint(pathToPointCommandChooser.getSelected())
              .alongWith(
                  new DeployAndIntake(true)
                      .andThen(Lighting.getStrobeCommand(() -> LEDState.kRed))
                      .finallyDo(Lighting.startTimer)));
    }
  ...
  public void testInit() {
    ...
    if (!testConfigured) {
      ...
      swerveCommands.setDefaultOption(
          "Do Nothing (Reset Gyro)",
          Drivetrain.getInstance()
              .runOnce(
                  () ->
                      Drivetrain.getInstance()
          ...
          Drivetrain.getInstance()
              .applyRequest(
                  () ->
                      new SwerveRequest.FieldCentric()
                          .withRotationalRate(
                              Constants.SwerveConstants.SwerveSpeeds
                                      .kMaxAngularSpeedRadiansPerSecond
                                  * swerveTestSpeed))
              .withName("Rotate"));

      testTab
          .add("Swerve Commands", swerveCommands)
          .withSize(9, 1)
          .withPosition(2, 2)
          .withWidget(BuiltInWidgets.kSplitButtonChooser);
      ...
      testTab
          .add("Arm Commands", armCommands)
          .withSize(5, 1)
          .withPosition(2, 1)
          .withWidget(BuiltInWidgets.kSplitButtonChooser);

      testTab
          .add("Extend Climber", new Extend())
          .withSize(2, 1)
          .withPosition(9, 0)
          .withWidget(BuiltInWidgets.kCommand);

      testTab
          .add("Retract Climber", new Retract())
          .withSize(2, 1)
          .withPosition(9, 1)
          .withWidget(BuiltInWidgets.kCommand);

      testTab
          .add("Intake (with deploying)", new DeployAndIntake(true))
          .withSize(2, 1)
          .withPosition(7, 0)
          .withWidget(BuiltInWidgets.kCommand);

      testTab
          .add("Intake (without deploying)", new DeployAndIntake(false))
          .withSize(2, 1)
          .withPosition(7, 1)
          .withWidget(BuiltInWidgets.kCommand);

      testTab
          .add("Desired Setpoint", Constants.ArmConstants.SetPoints.kIntake)
          .withSize(3, 1)
          .withPosition(2, 0)
          .withWidget(BuiltInWidgets.kTextView)
          .withProperties(Map.of("show_submit_button", true));

      desiredSetpointEntry =
          NetworkTableInstance.getDefault()
              .getTable("Shuffleboard")
      ...
      Alert.groups.forEach(
          (group, alert) -> {
            testTab.add(group, alert).withSize(2, 3).withPosition(0, 0).withWidget("Alerts");
          });
   ...
   *
   * @return will only return true if the robot is set to red, otherwise it returns false (blue)
   */
  ...
  /**
   * Will return false if the motor is verified and connected, true if there is some error getting
   * position
   *
  ...
  public static boolean verifyMotor(TalonFX falcon) {
    ...
    if (status.isError() && Robot.isReal()) {
      DataLogManager.log(
          "TalonFX ID #"
              + falcon.getDeviceID()
              + " has failed to return position with status: "
  ...
  /**
   * Will return false if the CANcoder is verified and connected, true if there is some error
   * getting position
   *
